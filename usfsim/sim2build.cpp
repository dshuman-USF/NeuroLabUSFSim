...
read_scr3 (FILE *f, int launch)
{
  ...
  if (fscanf (f, "%d %d %d %f %f ", &i, &p, &r, &t, &s) == 5) {
    if (i == 0)
      i = 1;
    if (p == 0)
      p = 1;
...
void V6CellAdjustments()
{
   ...
   {
      for (int node = 0; node < MAX_INODES; ++node) 
      {
         if (D.inode[node].node_type == CELL) 
         {
...
printf("%d\n",strcmp (filetype, ".sim"));
  ...
  for (int launch = 0; launch < MAX_LAUNCH; ++launch)
  {
     ...
     sprintf(header,"scr%d\n",pos);
     fprintf (f,"%s",header);
     fprintf (f, "%d %d %d %f %f\n", sp_aid[launch], sp_apop[launch], sp_arate[launch], sp_atk[launch], sp_ascale[launch]);
     putc ('\n', f);
     ++pos;
  }
  fclose (f);
...
void Save_sim (char *savename, char* sndname)
{
  int n;
