...
void SimViewer::setupViewer()
{
   ...
   stream << "Launch Instance is " << launchNum << endl;
...
void SimViewer::runConnRqst()
{
   ...
   connect(runSocket, QOverload<QAbstractSocket::SocketError>::of(&QAbstractSocket::error), this, &SimViewer::runSocketError);
}
...
//         plot->render(&painter,QRect(),rec,Qt::IgnoreAspectRatio);
      ...
      else
      {
         QString msg;
         QTextStream stream(&msg);
         stream << "Unable to create .pdf document." << endl;
         ui->infoBox->appendPlainText(msg);
      }
   }
}
...
void SimViewer::doChDir()
{
   ...
   if (fdlg.exec())
   {
      ...
      stream << "Changing directory to " << dir[0] << endl
             << "Note: No longer looking for new files." << endl
             << "Use the Restart button to resume looking for files" << endl
             << "or load existing ones." << endl;
      if (useSocket)
      {
         stream << "NOTE: Currently in \"Send Plot Data Directly To Simviewer\" mode. " << endl
                << "Switching to \"Plot Data To Files\" mode. There is no way back from this." << endl;
         useSocket = false;
         useFiles = true;
      }
      ui->infoBox->appendPlainText(msg);
...
void SimViewer::doRestart()
{
   ...
   stream << "Restarting using launch number " << launchNum << endl;
   ui->infoBox->appendPlainText(msg);
