...
{
#if 0
// this for webassembly experiments
  auto fileContentReady = [this](const QString &fileName, const QByteArray &fileContent) 
  {
    ...
    {
       this->printMsg(fileName);
       QFileDialog::saveFileContent(fileContent);
        // Use fileName and fileContent
    }
};
QFileDialog::getOpenFileContent("Models (*.snd)",  fileContentReady);
#endif
...
void SimWin::saveSim()
{
   ...
   if (fName.length())
   {
      ...
      launch->saveSnd();
      Save_sim(currSimFile.toLatin1().data(),currSndFile.toLatin1().data());
      QDir::setCurrent(info.path());
   }
}
